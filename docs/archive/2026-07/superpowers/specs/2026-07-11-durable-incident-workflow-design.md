...
- `id`、`case_id`
- `cycle_number`
- `phase`: `validation | investigation | fix | regression | legacy`
...
`legacy` 只用于迁移旧 `runs.json`，不允许由新流程创建或执行。
...
- `runs.json` 一次性迁移、重复迁移和损坏输入测试。旧记录统一导入为只读 `legacy_archived` Case 和 `legacy` attempt，不根据自由文本猜测可自动恢复的阶段；用户选择继续时从新 cycle 的 `pending_validation` 开始。
