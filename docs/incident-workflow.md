...
- 显式关闭活动 dialog、drawer 或 popover 时，Worker 先执行普通点击；若控件已唯一定位但因动画、遮罩或可操作性超时失败，Host 会在同一活动浮层内依次尝试受限强制点击、安全关闭控件和全局 `Escape`，并以浮层确实消失作为成功条件。BrowserPlan v2 只允许 `Escape` 在无 locator 时作为活动浮层关闭动作；其他全局按键仍被拒绝。
