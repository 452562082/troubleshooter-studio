...
- Worker 对真实 JSON 响应自动生成 `response_facts`：保留有界、非凭据的 `sample_values` 和 `values_truncated` 标记，并记录字段、数组、唯一值、相等/计数关系、数组内数值/时间字段的升降序，以及同一接口在不同因果动作下最大数值/时间的相对大小。最终判定必须用实际样本核对账号、用户、计数和时间等业务值；Token、Cookie、密码等凭据仍不落盘，完整原始响应体仍不保存。
- locator 失败是现场观察，不自动等于系统失败。Agent 可以根据冻结截图、页面结构和 Network 直接判定结果、询问用户，或生成有证据依据的后续动作；只有协议、进程或运行时故障才走系统重试。
- locator/action 自身失败不能作为 `insufficient_info` 结论；修复调用必须先根据冻结页面证据返回新的可执行策略。Host 会按失败动作记录完整因果交互链指纹，已经失败的策略不能在 role/text 等定位方式之间循环后再次执行。Worker 不按业务文案改写动作，而是优先在当前明确处于打开状态的顶层弹窗、抽屉或浮层内解析真实可见控件；不同控件族独立限额扫描，唯一最佳候选才允许执行。修复耗尽后仍只有执行缺口时归为系统故障，不向用户索要下游页面证据。
...
- 显式关闭活动 dialog、drawer 或 popover 时，BrowserPlan v2 使用无 locator 的 Host-owned `dismiss_surface`。Worker 冻结当前活动浮层，依次尝试 `Escape` 和至多一个安全关闭控件，并以该精确浮层确实消失作为成功条件；底层父浮层不得被误关。存量 v2 无 locator `Escape` 兼容走同一路径，其他全局按键仍被拒绝。
- Worker 将点击遮挡、可操作性超时、元素替换、元素不稳定和禁用状态保存为独立脱敏错误码，避免所有 Playwright 异常都退化为 `browser_action_failed`。
...
- 用户回复会成为最新场景澄清；validation 和 regression 都强制作废旧 recipe，并在同一 Case 的新 attempt 中重新生成完整 `scenario_contract` 和 BrowserPlan。若新计划已经采用该澄清但仍只发生页面执行失败，后续入口改为重新观察并继续，不得再次生成相同的兜底问题。
