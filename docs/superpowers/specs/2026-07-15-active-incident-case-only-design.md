...
- 详情读取失败继续写入现有可恢复错误区域；用户可使用当前 Case 标题区刷新重试。
...
6. `BugCaseArtifacts` 即使收到带 `reset_from_case_id` 或 `superseded_by_case_id` 的详情，也不渲染“重置关系”或归档链接。
7. 现有过程时间线、主阶段操作、阶段输出和证据卡测试保持通过。
8. 响应式用例覆盖 375px、768px、1024px、1440px 的单列布局和无横向溢出契约。
9. 运行相关组件与页面聚焦测试、Web 全量测试、`vue-tsc --noEmit` 和生产构建。
