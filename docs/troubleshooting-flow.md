...
生成物中的 `incident-investigator` 负责单次排障取证；Studio 桌面工作台负责把验证、方案评估、代码修复或非代码处置、合并、人工部署和回归组成可恢复的完整 Case。SQLite 是 Case 快照和追加式事件的真源，Agent 只执行一个阶段，Wails/UI、Agent 回调和恢复任务都不能绕过 `CaseOrchestrator` 改状态。
...
```text
reproduced -> investigating -> root_cause_ready
...
| 阶段 | 执行者 | 推进门槛 |
|---|---|---|
| 验证 | validator 角色（`reproduce`）+ Studio HostVerifier | 有可复现结论；Web 结论还必须有本轮最终渲染截图 |
| 排障 | 排障 Agent | 唯一高置信根因且无关键证据缺口 |
| 修复 | 修复 Agent | 用户批准“启动修复”，授权绑定根因 attempt 和 Case 版本 |
| 处置 | 人工/外部平台 | 非代码根因；用户登记实际执行摘要和证据，Studio 不自动执行运行时写操作 |
| 合并 | Studio Git service | 用户另行批准环境分支合并，授权绑定精确 commit、分支和目标 HEAD |
| 部署 | 人工/外部平台 | Studio 只等待通知，不执行部署 |
| 回归 | validator 角色（`regression`）+ Studio HostVerifier | 复用原始场景采集本次 attempt 的新鲜证据；运行版本若可自动采集则作为增强证据 |

Web validation / regression 不依赖 Agent CLI 是否能直接启动 Chromium。Studio 按以下固定协议持有浏览器执行和证据边界：
...
BrowserPlan 只描述 allowlist 内的导航、点击、填充、按键、选择、受控文件上传、等待和截图；文本断言仅允许 `visible_text`（必须出现）和 `not_visible_text`（不得出现），用于同时覆盖“应展示”和“未展示/不应展示”类 Bug。`upload_file` 只能引用当前 Case 原始业务附件或用户在补充信息中显式上传的测试文件 ID；Agent 看不到宿主路径，HostVerifier 校验文件名、MIME、大小和 SHA256，并在一次 Worker 生命周期内使用权限受限的临时文件。没有受控输入时返回 `insufficient_info`，不能跳过上传步骤或伪造路径。

任意 JavaScript、凭据、Cookie、任意 header、宿主路径、登录模式上传和生产写交互仍然禁止。HostVerifier 冻结截图、脱敏 Network / console 与 `browser-actions.json` 后再交给 evaluator，validator 不能重写 artifact 路径。登录缺失以 `browser_login_required` 停在 `waiting_evidence`，由用户在 Studio 可见浏览器完成登录；Studio 加密保存 session 并用父链 attempt 继续，不在 Case 文本中收账号密码。结构校验失败、浏览器系统错误或一次有限定位修复仍失败时，可在当前 Case 内重试或重新观察页面生成计划，不需要重建故障闭环。

“已部署”按钮和明确的自然语言通知走同一幂等 verifier 路径。`准备部署`、`还没部署`、`部署失败`等表达不会触发校验。HTTP 或 K8s 版本来源明确读到与目标 commit 不一致时，Case 保持未验证；未配置、不可读、未暴露或只能部分识别时记录 `unavailable` 诊断，以空版本继续回归。用户不需要手工填写版本或 commit。

### 证据新鲜度

证据属于具体 Case、cycle 和 attempt，发布前由 Studio 放入 attempt 专属暂存目录并计算 SHA256。回归不能复用首次验证的文件冒充结果；`fixed_verified` 和 `still_reproduces` 都必须引用当前回归 attempt 在启动后采集、环境与已验证部署一致的新证据，并拒绝历史 request ID、trace ID 或相同摘要。Web 结论还必须绑定 HostVerifier 确认的本轮最终 PNG；除登录表单禁止截图外，执行失败则保留失败现场截图和明确步骤。版本观测也必须绑定当前 cycle 的 merge approval 和部署 reservation。
...
| Bug 验证/回归 | `bug-verifier` |
| 附件证据 | `attachment-evidence-verifier` |
| API 验证 | `api-verifier` |
| 前端复现 | `frontend-repro-investigator` |
| 代码智能 | `code-intelligence-query` |
| 代码修复 | `bug-fixer` |
| 服务拓扑 | `service-topology-query` |
| 路由 | `routing` |
| 最近变更 | `recent-changes` |
| 配置 | `config-executor` |
...
| 日志 | `elk-log-query` |
| 数据 | `mongodb-runtime-query`、`mysql-runtime-query`、`postgresql-runtime-query`、`redis-runtime-query`、`es-runtime-query`、`clickhouse-runtime-query`、`kafka-runtime-query`、`rabbitmq-runtime-query` |
...
客户端复现问题先走 `frontend-repro-investigator`：HAR/console/RUM 任一证据 → 提取失败请求/trace_id/静态资源问题 → 再接 backend trace/log/runtime；没有浏览器证据时不得只凭后端 trace 下结论。在 Studio Case 中，Web 证据由 HostVerifier 正式采集，`frontend-repro-investigator` 负责归一化和后端交接；其 `browser_collect.mjs` 只供脱离持久化 Case 的手动兼容采集，不能冒充 Studio 权威截图。
