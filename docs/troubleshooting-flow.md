...
| 8 | Step 6 加第 8 条结论自检 runtime 配置源优先级(配置中心 > 代码 config.yaml,防把代码里过时默认地址当 runtime 真值) | `7a7750e` |
| 9 | 补强 #8 fallback 规则:nacos-mcp-router 调失败时**禁止 silent fallback 到代码 config.yaml 当 runtime 真值**,走 ASK_USER 三分支;头注释含"本地/开发/fallback/default/示例"的 yaml 降级为参考值绝不当唯一证据 | _本次_ |
