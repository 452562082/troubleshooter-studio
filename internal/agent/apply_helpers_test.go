...
func TestLooksLikeFactoryArtifact_LocalYamlPreserved(t *testing.T) {
	cases := []struct {
		...
		// 正例:本来 .local.yaml 用例
		{"known-errors.local.yaml in skills/routing/references", "skills/routing/references/known-errors.local.yaml", "claude-code", false},
		...
		{"known-errors.yaml IS factory", "skills/routing/references/known-errors.yaml", "claude-code", true},
		// 不带 .local.yaml 后缀的不受影响
		{"plain skills/ file", "skills/routing/SKILL.md", "claude-code", true},
		// .local.yaml 但不在 skills/ 下也不删
		{"top-level .local.yaml", "my-stuff.local.yaml", "claude-code", false},
	}
	for _, c := range cases {
