...
func (c BrowserCoordinator) Execute(ctx context.Context, request BrowserCoordinatorRequest) (BrowserCoordinatorResult, error) {
	...
	if !found && c.Recipes != nil && !browserForceReplan(request.Attempt) {
		...
		if recipeFound && recipe.ScenarioSHA256 == scenarioSHA {
			...
			if err := validateBrowserPlanStartOrigin(plan, request.Policy); err != nil {
				return browserCoordinatorPlanFailure(result, err), nil
			}
...
locatorRecovery:
	for currentResult.Status == "locator_failed" {
		...
		repairEvidence, evidenceErr := parseFrozenBrowserStructuredEvidence(currentFrozen)
		if evidenceErr != nil {
		...
		if repairFound {
			...
			statuses, statusErr := browserEvaluatorStatuses(request.Attempt)
			if statusErr != nil {
				_ = cleanupRepairEvidence()
				return browserCoordinatorFailure(result, "browser_execution_interrupted"), nil
			}
			repairPrompt += browserLocatorDecisionContract(statuses)
			...
			if repairErr != nil {
				...
				if browserValidatorErrorCode(repairErr) == "browser_validator_timeout" {
					if request.Emit != nil {
						request.Emit(InvestigationEvent{
							Type:    "browser_locator_adjudication_fallback",
							Message: "页面策略调整超时，正在直接根据已采集证据判定结果",
						})
					}
					break locatorRecovery
				}
	...
	if err != nil && browserValidatorErrorCode(err) == "browser_validator_timeout" && ctx.Err() == nil {
		if request.Emit != nil {
			request.Emit(InvestigationEvent{
				Type:    "browser_evaluator_timeout_retry",
				Message: "验证结果判定超时，正在基于同一份冻结证据自动重试一次",
			})
		}
		retryPrompt := evaluatorPrompt + "\n\nThe previous adjudication call timed out before returning a result. Use only the same frozen evidence above. Return the strict ValidationResult immediately; do not perform additional exploration.\n"
		...
		addAgentUsage(&retry.Usage, evaluation.Usage)
		evaluation, err = retry, retryErr
	}
...
func enforceBrowserResponseAssertionOutcome(attempt PhaseAttempt, current BrowserVerificationResult, frozen []browserFrozenArtifact, validation *ValidationResult) error {
	...
	if violations > 0 {
		...
		switch {
		case statusAssertions > 0 && fieldAssertions == 0:
			validation.ObservedBehavior = fmt.Sprintf("接口请求本应在当前请求阶段被拒绝，但机器校验发现 %d 个因果请求返回了成功状态（共匹配 %d 个请求）。", violations, matched)
		case statusAssertions == 0:
			validation.ObservedBehavior = fmt.Sprintf("接口响应字段关系机器校验发现 %d 个不符合预期的对象（共匹配 %d 个对象）。", violations, matched)
		default:
			validation.ObservedBehavior = fmt.Sprintf("接口响应机器校验发现 %d 个违反项（共匹配 %d 个请求或对象）。", violations, matched)
		}
	...
	if unmatched > 0 {
		...
		switch {
		case statusAssertions > 0 && fieldAssertions == 0:
			validation.ObservedBehavior = fmt.Sprintf("接口请求结果机器校验有 %d 项未匹配到当前操作触发的因果 HTTP 响应。", unmatched)
			validation.Gaps = []string{"未匹配到当前提交或上传操作触发的目标 HTTP 响应。"}
		case statusAssertions == 0:
		...
		default:
			validation.ObservedBehavior = fmt.Sprintf("接口响应机器校验有 %d 项未匹配到目标响应或 JSON 对象。", unmatched)
			validation.Gaps = []string{"未匹配到全部声明的接口响应证据。"}
		}
		return nil
	}
	...
	switch {
	case statusAssertions > 0 && fieldAssertions == 0:
		validation.ObservedBehavior = fmt.Sprintf("接口请求拒绝机器校验通过，共匹配 %d 个因果请求且均未返回成功状态。", matched)
	case statusAssertions == 0:
		validation.ObservedBehavior = fmt.Sprintf("接口响应字段关系机器校验通过，共匹配 %d 个对象且未发现违反项。", matched)
	default:
		validation.ObservedBehavior = fmt.Sprintf("接口响应机器校验通过，共匹配 %d 个请求或对象且未发现违反项。", matched)
	}
	validation.Gaps = []string{}
	return nil
}
...
func browserExpectedScenarioText(request BrowserCoordinatorRequest) string {
	for index := len(request.UserClarifications) - 1; index >= 0; index-- {
		if clarification := strings.TrimSpace(request.UserClarifications[index]); clarification != "" {
...
func browserPlanHasResponseAssertionKind(plan BrowserPlan, kind string) bool {
...
type browserPlanValidationDiagnostic struct {
	Code    string
	Message string
}

func browserCoordinatorPlanFailure(result BrowserCoordinatorResult, err error) BrowserCoordinatorResult {
	result = browserCoordinatorFailure(result, "browser_validator_plan_invalid")
	result.FailureStage = "plan_validation"
	diagnostic := browserPlanValidationDiagnosticFor(err)
...
func browserPlanValidationDiagnosticFor(err error) browserPlanValidationDiagnostic {
	message := ""
	if err != nil {
		message = strings.ToLower(err.Error())
	}
	...
	case strings.Contains(message, "frontend_entry_ids must preserve"):
		return browserPlanValidationDiagnostic{Code: "frontend_scope_order_invalid", Message: "场景合同中的应用端顺序与用户选择不一致"}
	case strings.Contains(message, "does not visit selected frontend entry"):
		return browserPlanValidationDiagnostic{Code: "frontend_entry_not_visited", Message: "计划没有访问全部已选择的应用端"}
	case strings.Contains(message, "no causal evidence action for selected frontend entry"):
		return browserPlanValidationDiagnostic{Code: "frontend_evidence_incomplete", Message: "至少一个已选择的应用端没有形成可验证证据"}
	case strings.Contains(message, "device_profile"):
		return browserPlanValidationDiagnostic{Code: "device_profile_mismatch", Message: "计划设备类型与当前验证场景不一致"}
	case strings.Contains(message, "scenario_contract"):
		return browserPlanValidationDiagnostic{Code: "scenario_contract_invalid", Message: "场景合同缺失、引用不完整或不属于当前用户澄清"}
	case strings.Contains(message, "search page entry"):
		return browserPlanValidationDiagnostic{Code: "search_entry_unresolved", Message: "计划没有使用当前页面中可确认的搜索入口"}
	case strings.Contains(message, "browser origin is not allowed"),
		strings.Contains(message, "start origin"),
		strings.Contains(message, "start_url"):
		return browserPlanValidationDiagnostic{Code: "frontend_origin_invalid", Message: "计划入口不属于本次已配置的应用范围"}
	...
	case strings.Contains(message, "action"), strings.Contains(message, "assertion"),
		...
		return browserPlanValidationDiagnostic{Code: "plan_structure_invalid", Message: "动作、断言或必填字段不符合浏览器计划协议"}
	default:
		return browserPlanValidationDiagnostic{Code: "plan_structure_invalid", Message: "计划未通过严格结构校验，原始输出已按安全规则隐藏"}
	}
}
...
func browserCoordinatorAgentAssistance(result BrowserCoordinatorResult, raw, stage string) (BrowserCoordinatorResult, bool) {
	request, err := ParseBrowserAssistanceRequest([]byte(raw))
	if err != nil {
		return result, false
	}
	result = browserCoordinatorFailure(result, "browser_validation_needs_user_input")
	result.FailureStage = safeBoundedBrowserText(stage, 64)
	result.ValidationQuestions = append([]BrowserValidationQuestion(nil), request.Questions...)
	return result, true
}
...
func browserCoordinatorAgentConclusion(request BrowserCoordinatorRequest, result BrowserCoordinatorResult, plan BrowserPlan, raw string, frozen []browserFrozenArtifact) (BrowserCoordinatorResult, bool) {
	...
	if err := enforceBrowserResponseAssertionOutcome(request.Attempt, result.BrowserResult, frozen, &validation); err != nil {
		return result, false
	}
	if err := enforceBrowserRequestFactCompleteness(result.BrowserResult, frozen, &validation); err != nil {
		return result, false
	}
	validation.Evidence = browserArtifactReferences(result.BrowserArtifacts)
	if err := validateValidationResult(validation); err != nil {
...
func browserValidationQuestions(result BrowserCoordinatorResult) []map[string]string {
	if len(result.ValidationQuestions) != 0 {
		questions := make([]map[string]string, 0, len(result.ValidationQuestions))
		for _, question := range result.ValidationQuestions {
			questions = append(questions, map[string]string{
				"id":          safeBoundedBrowserText(question.ID, 64),
				"question":    safeBoundedBrowserText(question.Question, 1000),
				"answer_hint": safeBoundedBrowserText(question.AnswerHint, 1000),
			})
		}
		return questions
	}
...
func browserPassiveDowngradeAuthorizationFor(original BrowserPlan, failed BrowserVerificationResult, evidence browserEvaluatorEvidence) browserPassiveDowngradeAuthorization {
	...
	failedAction := original.Actions[failedIndex]
	if failedAction.Action != "click" && failedAction.Action != "press" {
		return browserPassiveDowngradeAuthorization{}
	}
	triggerAction := original.Actions[failedIndex-1]
	if !browserStateChangingAction(triggerAction.Action) {
		return browserPassiveDowngradeAuthorization{}
	}
	...
	for _, action := range evidence.BrowserActions {
		switch {
		case action.ID == triggerAction.ID && action.Action == triggerAction.Action && action.Result == "completed":
			triggerCompleted = true
		case action.ID == failedAction.ID && action.Action == failedAction.Action && action.Result == "failed":
			failedObserved = true
		}
	}
...
func browserPassiveWaitLocatorGrounded(locator *BrowserLocator, accessibility []BrowserAccessibilityNode) bool {
	if locator == nil || locator.Exact == nil || !*locator.Exact {
		return false
	}
	kind := strings.ToLower(strings.TrimSpace(locator.Kind))
	value := strings.TrimSpace(locator.Value)
	name := strings.TrimSpace(locator.Name)
	if value == "" {
		return false
	}
	for _, node := range accessibility {
		if !node.Visible {
			continue
		}
		switch kind {
		case "role":
			if name != "" && strings.EqualFold(strings.TrimSpace(node.Role), value) && strings.TrimSpace(node.Name) == name {
				return true
			}
		case "text":
			if strings.EqualFold(strings.TrimSpace(node.LocatorKind), kind) && strings.TrimSpace(node.Name) == value {
				return true
			}
			if strings.EqualFold(strings.TrimSpace(node.Role), "document") {
				for _, line := range strings.Split(node.Name, " · ") {
...
func validateBrowserRepairWithEvidence(original BrowserPlan, failed BrowserVerificationResult, evidence browserEvaluatorEvidence, repaired BrowserPlan) error {
	...
	for index := range want {
		...
		if before.Action != after.Action {
			evidenceBoundPassiveDowngrade := index == failedIndex &&
				passiveDowngrade.Allowed &&
				passiveDowngrade.FailedActionID == before.ID &&
				(before.Action == "click" || before.Action == "press") &&
...
func browserRepairPrompt(original BrowserPlan, failed BrowserVerificationResult, evidence browserEvaluatorEvidence, observation *BrowserVerificationResult, screenshotManifest []string) string {
	...
	report := map[string]any{
		...
		"passive_downgrade": browserPassiveDowngradeAuthorizationFor(original, failed, evidence),
	}
	if observation != nil {
...
func browserEvaluatorPrompt(request BrowserCoordinatorRequest, result BrowserVerificationResult, artifacts []BrowserArtifactReference, frozen []browserFrozenArtifact) (string, []PhaseAttachment, func() error, error) {
	...
	prompt := "Evaluate the browser verification evidence. Output only the strict ValidationResult YAML contract below.\n" +
		...
		"response_facts is trusted neutral observation evidence automatically generated during the current browser execution. It contains JSON field paths, occurrence/unique-value counts, bounded non-credential sample_values, values_truncated markers, array lengths, equal-field-pair counts, count-field/array-length relations, array field_orders, and cross_response_comparisons between the maximum numeric or datetime field observed for two causal actions. Use sample_values to compare the actual current account, user IDs, names, counts, timestamps, and other business values across causal responses; never infer an account mismatch or missing prerequisite when those values can decide it. A field_order direction establishes the real response-array order. A cross_response_comparison establishes the named left_action_id/right_action_id relation for the same endpoint and field_path; combine it with each action's selected UI target, screenshots, sample values, and zero-length arrays to evaluate latest-value ordering and empty-result placement. sample_values are bounded and credential-filtered rather than a complete raw response body; values_truncated=true means the samples are incomplete. Do not require a raw response body or a second validation pass when response_facts already establishes the needed business value, structure, equality, or ordering fact.\n" +
