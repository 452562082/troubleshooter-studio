...
func TestBuildPlan_SkippedSkills(t *testing.T) {
	...
	if len(plan.SkillsIncluded) != 5 {
		t.Errorf("expected 5 included skills (2 whitelist + fixer + 4 validator baseline + observability evidence), got %d: %+v", len(plan.SkillsIncluded), plan.SkillsIncluded)
	}
	for _, want := range []string{"routing", "diagram-generator", "bug-fixer", "frontend-repro-investigator", "grafana-observability-query"} {
		found := false
