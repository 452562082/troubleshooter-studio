...
describe('BugWorkflowMetrics', () => {
  ...
  it('shows compact outcome, wait and blocker metrics', () => {
    ...
    expect(wrapper.text()).toContain('已完成工作台处理 5')
    expect(wrapper.text()).not.toContain('回归')
    expect(wrapper.findAll('.metric-grid div').map(item => [item.find('dt').text(), item.find('dd').text()])).toContainEqual(['排障', '2小时'])
    expect(wrapper.findAll('.metric-grid div').map(item => [item.find('dt').text(), item.find('dd').text()])).toContainEqual(['提交', '0秒'])
    expect(wrapper.text()).toContain('合并冲突 1')
  })
})
