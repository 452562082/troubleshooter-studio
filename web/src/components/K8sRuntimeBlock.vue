...
const props = defineProps<{
  envID: string
  services: string[]
  /** k8s_runtime provider:"kuboard" 或 "one2all" */
  provider?: string
...
const emit = defineEmits<{
  ...
  setEnvLoc: [envID: string, field: 'cluster' | 'cluster_id' | 'namespace', value: string]
...
<template>
  <div class="loki-env-mapping">
    ...
    <div
      v-if="(isOne2All && clusterId && namespace && services.length > 0)
        || (!isOne2All && cluster && namespace && services.length > 0)"
      class="cc-map-block"
    >
