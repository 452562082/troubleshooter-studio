...
import { setLokiDatasource, type LokiMappingPerEnv } from './useLokiMappingState'
...
export function useGrafanaDS(deps: UseGrafanaDSDeps) {
  ...
  async function loadLokiDatasources(envID: string) {
    const lm = deps.getLokiMapping(envID)
    const revision = revisions[envID] = (revisions[envID] || 0) + 1
    const auth = lokiAuthFor(envID)
    const connection = (a: ReturnType<typeof lokiAuthFor>) => JSON.stringify([a.grafana_url, a.api_key, a.user, a.pass])
    const stale = () => disposed || revisions[envID] !== revision || connection(auth) !== connection(lokiAuthFor(envID))
    ...
    try {
      ...
      lm.dsList = list
      lm.dsListStatus = 'ok'
      const lokis = list.filter(d => d.type === 'loki')
      if (!lokis.some(d => d.uid === lm.dsUID)) setLokiDatasource(lm, lokis.length === 1 ? lokis[0].uid : '')
      for (const obsKey of ['prometheus', 'jaeger', 'tempo', 'elk']) {
        ...
        if (candidates.some(d => d.uid === grafanaDsUidByObsEnv[k])) continue
        grafanaDsUidByObsEnv[k] = candidates.length === 1 ? candidates[0].uid : ''
      }
      ...
      if (!disposed && revisions[envID] === revision && connection(auth) !== connection(lokiAuthFor(envID))) {
        lm.dsListStatus = 'idle'
        lm.dsList = []
      }
    }
  }
  ...
  function scheduleGrafanaDsAutoload(envID: string) {
    ...
    const lm = deps.getLokiMapping(envID)
    lm.dsListStatus = 'idle'
    if (!auth.grafana_url || (!auth.api_key && (!auth.user || !auth.pass))) return
    lm.dsListStatus = 'loading'
  ...
  if (getCurrentScope()) onScopeDispose(() => {
    disposed = true
    for (const timer of Object.values(grafanaDsAutoTimers)) clearTimeout(timer)
  })
